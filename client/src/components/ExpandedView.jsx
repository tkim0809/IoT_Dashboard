...
const ExpandedView = ({ fleetName, data, onClose }) => {
    ...
    return (
        <div className="p-5 rounded-lg border-solid border-black border-2 bg-white top-1/2 left-1/2" style={{ width: "90%", height: "91%", position: "fixed", overflowY: "auto",   transform: "translate(-50%, -50%)"}}>
            ...
            <thead style={{backgroundColor: "#86a7b5"}}> 
                ...
                <tbody>
                        {mockData.map((data, index) => (
                            <tr>
                            <td className={`${tableD} table_d`} style={{backgroundColor: index % 2 === 0 ? "white" : "#ecf2f3"}}>{data.device}</td>
                            <td className={`${tableD} table_d`} style={{backgroundColor: index % 2 === 0 ? "white" : "#ecf2f3"}}>{data.uploadDate}</td>
                            <td className={`${tableD} table_d`} style={{backgroundColor: index % 2 === 0 ? "white" : "#ecf2f3"}}>{data.lastCampaign}</td>
                            <td className={`${tableD} table_d`} style={{backgroundColor: index % 2 === 0 ? "white" : "#ecf2f3"}}>{data.status}</td>
                            <td className={`${tableD} table_d`} style={{backgroundColor: index % 2 === 0 ? "white" : "#ecf2f3"}}>{data.failedContent}</td>
                            <td className={`${tableD} table_d`} style={{backgroundColor: index % 2 === 0 ? "white" : "#ecf2f3"}}>{data.content}</td>
                            </tr>
                        ))}
                </tbody>
            </table>
